...
// 6. Relevant Functions Execution Control
    $getAllVehicleEmissionTestsResults = $LicenseAndPermitObj->getAllVehicleEmissionTests();
...
?>
    ...
    ?> 
            ...
            <div class="row">
                <h4 style="text-align:center"><?php echo $getSpecificFunctionRelevatFunctionIdVehicleEmissionTests["functions_name"];?></h4>
            </div>           
            <div class="row">
            ...
            <div class="row">
                <table class="table table-hover table-bordered" id="drivertable">
                    ...
                    <tbody>
    <?php
                    while($getAllVehicleEmissionTests = $getAllVehicleEmissionTestsResults->fetch_assoc()){
                        $selectedImg = "";
                            if($getAllVehicleEmissionTests["vehicleImage_"]=="")
                            {
                                $selectedImg = "../../images/driver_images/download.jpeg";
                            }
                            else{
                                $selectedImg = "../../images/vehicle_images/".$getAllVehicleEmissionTests["vehicleImage_"];
                            }
...
//    ?>  
                        <tr>
                            ...
                            <td scope="row">
                                <a href="specific-vehicle-emission-tests.php?vehicle_emission_tests_id=<?php echo $getAllVehicleEmissionTests["vehicle_emission_tests_id"];?>&module_id=<?php echo $module_id;?>&functions_idVehicleEmissionTests=<?php echo $functions_idVehicleEmissionTests;?>&functions_idSpecificVehicleEmissionTests=108">
                                    <img class="rounded-circle" src="<?php echo $selectedImg ?>" width="30px" height="30px"/>
                                </a>
                            </td>
                            <td><?php echo ucwords($getAllVehicleEmissionTests["make"]." ".$getAllVehicleEmissionTests["model"]);?></td>
                            <!--<td><?php echo ucwords($driverrow["email"]); ?></td>-->
                            <td><?php echo ucwords($getAllVehicleEmissionTests["vehicle_emission_test_centreName"]); ?></td>
                            <td><?php echo ucwords($getAllVehicleEmissionTests["class_of_vehicle_name"]); ?></td>
                            <td><?php echo ucwords($getAllVehicleEmissionTests["vehicle_emission_test_validTill"]); ?></td>
